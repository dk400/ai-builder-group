...
--
--  생성: scripts 없이 목업 데이터에서 자동 변환 — 목업이 바뀌면 다시 만든다.
...
insert into public.builders (slug, name, email, role, one_liner, role_label, avatar_url, is_active) values
  ...
  ('ria', '빌더 리아', 'ria@example.com', 'builder', '수주용 랜딩 · 브랜드 사이트', '랜딩 · 인터랙션', '/assets/img/av-ria.jpg', true),
...
insert into public.works (slug, title, summary, category_id, thumb_url, hero_url, period_label, status, published_at, created_by, reject_reason) values
  ('샘플-작성중-프로젝트', '[샘플 ①] 작성 중인 프로젝트 — 자유롭게 수정할 수 있습니다', '아직 제출하지 않은 초안입니다. 제목 · 요약 · 분야 · 커버 · 참여 빌더를 모두 고칠 수 있고, 다 되면 검토를 요청합니다.', (select id from public.categories where type='work' and slug='saas-admin'), '/assets/img/work-aerok-admin.jpg', '/assets/img/work-aerok-admin.jpg', '2026', 'draft', null, (select id from public.builders where slug='ria'), null),
  ('샘플-승인대기-프로젝트', '[샘플 ②] 문의 응대를 한곳에 모은 사내 AI 어시스턴트', '세 채널로 흩어져 들어오던 문의를 하나의 받은편지함으로 모으고, 담당자가 고쳐 쓸 응대 초안까지 붙여 주는 내부 도구.', (select id from public.categories where type='work' and slug='ai-ax'), '/assets/img/work-canape.png', '/assets/img/work-canape.png', '2026', 'pending', null, (select id from public.builders where slug='ria'), null),
  ('샘플-반려된-프로젝트', '[샘플 ③] 반려된 프로젝트 — 사유를 보고 다시 고칩니다', '관리자가 사유와 함께 돌려보낸 상태입니다. 화면 맨 위에 사유가 붙고, 폼은 다시 열려 있습니다.', (select id from public.categories where type='work' and slug='commerce'), '/assets/img/work-markspon.png', '/assets/img/work-markspon.png', '2026', 'rejected', null, (select id from public.builders where slug='ria'), '커버에 고객사 로고가 그대로 남아 있습니다. 공개 동의를 확인하기 전까지는 지우거나 다른 컷으로 교체해 주세요(기획서 §14 Q7). 요약 두 번째 문장의 성과 수치도 출처가 필요합니다.'),
  ('샘플-발행된-프로젝트', '[샘플 ④] 발행된 프로젝트 — 공개 중, 내릴 수 있습니다', '승인되어 공개된 상태입니다. 작성자는 더 이상 손대지 못하고, 관리자만 수정하거나 내릴 수 있습니다.', (select id from public.categories where type='work' and slug='finance'), '/assets/img/work-nice.png', '/assets/img/work-nice.png', '2025', 'published', '2026-08-18T00:00:00Z', (select id from public.builders where slug='ria'), null),
  ('샘플-보관된-프로젝트', '[샘플 ⑤] 보관된 프로젝트 — 다시 공개할 수 있습니다', '공개에서 내려간 상태입니다. 지운 것이 아니라 보관입니다 — 관리자가 다시 공개할 수 있고, 주소는 목록으로 301 됩니다.', (select id from public.categories where type='work' and slug='media'), '/assets/img/work-btv.png', '/assets/img/work-btv.png', '2024', 'archived', null, (select id from public.builders where slug='ria'), null),
  ...
  ('o2o-예약-사용자앱', 'Aerok User — 사용자 앱', '예약·이용 플로우 전면 구축.', (select id from public.categories where type='work' and slug='o2o'), '/assets/img/work-aerok-user.jpg', '/assets/img/work-aerok-user.jpg', '2025', 'published', '2025-12-01T00:00:00Z', (select id from public.builders where slug='ria'), null),
  ...
  ('플랫폼-돌봄-연결', '패밀리케어 — 키즈노트', '가족 돌봄 연결 서비스.', (select id from public.categories where type='work' and slug='platform'), '/assets/img/work-familycare.jpg', '/assets/img/work-familycare.jpg', '2022', 'rejected', null, (select id from public.builders where slug='ria'), '히어로 이미지에 실제 이용자 얼굴이 그대로 보입니다. 마스킹하거나 다른 컷으로 교체 후 다시 제출해 주세요.')
on conflict do nothing;

-- ── Work ↔ 빌더 ─────────────────────────────────────────────────────
-- sort 0 이 리드다. 목록의 '참여 빌더' 열이 이 순서로 이름을 잇는다.
insert into public.work_builders (work_id, builder_id, role_label, sort) values
  ((select id from public.works where slug='샘플-작성중-프로젝트'), (select id from public.builders where slug='ria'), '리드', 0),
  ((select id from public.works where slug='샘플-작성중-프로젝트'), (select id from public.builders where slug='hajun'), '참여', 1),
  ((select id from public.works where slug='샘플-승인대기-프로젝트'), (select id from public.builders where slug='ria'), '리드', 0),
  ((select id from public.works where slug='샘플-승인대기-프로젝트'), (select id from public.builders where slug='dohyun'), '참여', 1),
  ((select id from public.works where slug='샘플-반려된-프로젝트'), (select id from public.builders where slug='ria'), '리드', 0),
  ((select id from public.works where slug='샘플-반려된-프로젝트'), (select id from public.builders where slug='taeo'), '참여', 1),
  ((select id from public.works where slug='샘플-발행된-프로젝트'), (select id from public.builders where slug='ria'), '리드', 0),
  ((select id from public.works where slug='샘플-발행된-프로젝트'), (select id from public.builders where slug='sein'), '참여', 1),
  ((select id from public.works where slug='샘플-보관된-프로젝트'), (select id from public.builders where slug='ria'), '리드', 0),
  ...
  ((select id from public.works where slug='o2o-예약-사용자앱'), (select id from public.builders where slug='ria'), '리드', 0),
  ...
  ((select id from public.works where slug='플랫폼-돌봄-연결'), (select id from public.builders where slug='ria'), '리드', 0),
...
insert into public.insights (slug, title, excerpt, body_html, thumb_url, category_id, author_id, status, published_at, reject_reason) values
  ('샘플-작성중-글', '[샘플 ①] 작성 중 — 자유롭게 수정할 수 있습니다', '아직 제출하지 않은 초안입니다. 제목·본문·썸네일·주소를 모두 고칠 수 있고, 다 되면 검토를 요청합니다.', '<h2 id="여기를-고쳐-보세요">여기를 고쳐 보세요</h2><p>툴바로 소제목·굵게·목록·링크를 넣어 볼 수 있습니다. H1 은 없습니다 — 페이지 제목이 h1 이라 본문은 H2 부터 시작합니다.</p><h2 id="다음-단계">다음 단계</h2><p>아래 <strong>검토 요청</strong> 을 누르면 ② 상태로 넘어가고, 그때부터 작성자는 수정할 수 없습니다.</p>', '/assets/img/ins/ins-turnkey.jpg', (select id from public.categories where type='insight' and slug='guide'), (select id from public.builders where slug='ria'), 'draft', null, null),
...
<div class="tags"><span class="tag">일하는 방식</span><span class="tag">프로덕트 빌더</span><span class="tag">협업</span></div>', '/assets/img/ins/ins-native.jpg', (select id from public.categories where type='insight' and slug='how'), (select id from public.builders where slug='ria'), 'pending', null, null),
  ('샘플-반려된-글', '[샘플 ③] 반려됨 — 사유를 보고 다시 고칩니다', '관리자가 사유와 함께 돌려보낸 상태입니다. 작성자는 사유를 보고 고친 뒤 다시 검토를 요청합니다.', '<h2 id="반려의-의미">반려의 의미</h2><p>거절이 아니라 되돌림입니다. 화면 맨 위에 사유가 붙어 있고, 폼은 다시 열려 있습니다.</p><h2 id="사유가-필수인-이유">사유가 필수인 이유</h2><p>사유 없는 반려는 “안 됨”만 전달합니다. 무엇을 고쳐야 하는지 없으면 다시 올라오는 것도 같은 상태입니다.</p>', '/assets/img/ins/ins-gov.jpg', (select id from public.categories where type='insight' and slug='how'), (select id from public.builders where slug='ria'), 'rejected', null, '두 번째 소제목의 주장에 근거가 없습니다. 실제로 겪은 사례나 수치를 한 줄 넣어 다시 제출해 주세요. 그리고 썸네일이 본문 내용과 맞지 않습니다.'),
  ('샘플-발행된-글', '[샘플 ④] 발행됨 — 공개 중, 내릴 수 있습니다', '승인되어 공개된 상태입니다. 작성자는 더 이상 손대지 못하고, 관리자만 수정하거나 내릴 수 있습니다.', '<h2 id="여기서부터는-관리자-몫">여기서부터는 관리자 몫</h2><p>공개 중인 글이 검수 없이 바뀌면 안 되기 때문입니다(PRD §7.3 편집 주체 = 관리자).</p><h2 id="내리면">내리면</h2><p><strong>내리기</strong> 를 누르면 ⑤ 보관 상태가 되고, 그 주소는 404 가 아니라 목록으로 301 됩니다 — 색인과 공유 링크를 버리지 않습니다.</p>', '/assets/img/ins/ins-ax.jpg', (select id from public.categories where type='insight' and slug='project'), (select id from public.builders where slug='ria'), 'published', '2026-08-18T00:00:00Z', null),
  ('샘플-보관된-글', '[샘플 ⑤] 보관됨 — 내려간 글, 다시 공개할 수 있습니다', '공개에서 내려간 상태입니다. 지운 것이 아니라 보관입니다 — 관리자가 다시 공개할 수 있습니다.', '<h2 id="지우지-않는-이유">지우지 않는 이유</h2><p>지우면 작성자 연결과 이력이 함께 사라집니다. 상태를 삭제 플래그로 겸하지 않는 것도 같은 이유입니다.</p><h2 id="주소는-살아-있다">주소는 살아 있다</h2><p>보관된 글의 주소는 목록으로 301 됩니다(DR-08).</p>', '/assets/img/ins/ins-toss.jpg', (select id from public.categories where type='insight' and slug='project'), (select id from public.builders where slug='ria'), 'archived', null, null),
...
<div class="tags"><span class="tag">발주 가이드</span><span class="tag">외주</span><span class="tag">체크리스트</span></div>', '/assets/img/ins/ins-turnkey.jpg', (select id from public.categories where type='insight' and slug='guide'), (select id from public.builders where slug='josh'), 'published', '2026-08-11T00:00:00Z', null),
  ('3주-랜딩페이지-제작순서', '우리가 3주 만에 랜딩 페이지를 만드는 순서', '기획·디자인·개발을 한 사람이 맡으면 일정이 어떻게 접히는지.', null, '/assets/img/ins/ins-native.jpg', (select id from public.categories where type='insight' and slug='how'), (select id from public.builders where slug='ria'), 'published', '2026-08-09T00:00:00Z', null),
