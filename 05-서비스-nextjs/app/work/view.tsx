...
import { WORKS, creditOf } from '@/app/_works'
import { BUILDERS, badgeOf, builderBySlug } from '@/app/_builders'

const pad2 = (n: number) => String(n).padStart(2, '0')

/* 프로젝트·빌더 데이터는 app/_works.ts · app/_builders.ts 에 있다.
   /builder 와 홈이 같은 배열을 보므로 여기서 다시 적지 않는다. */
export default function WorkView() {
  ...
  useEffect(() => {
    /* 추천 = [빌더 슬러그, 추천 이유]. 이름·역할·수행 건수는 _builders 에서 읽는다 —
       전에는 '9건 수행' 이 이 표에 박혀 있어서, 빌더 실적을 고쳐도 여기만 옛 숫자로 남았다. */
    const MATCH: Record<string, Array<[string, string]>> = {
      landing: [['ria', 'BEST'], ['minseo', '디자인']],
      platform: [['dohyun', 'BEST'], ['josh', 'LEAD']],
      ai: [['yuna', 'BEST'], ['sein', '자동화']],
      app: [['hajun', 'BEST'], ['ria', '디자인']],
    }
    let picked = 'ai'
    const go = (s: string) => {
    ...
    const renderPicks = () => {
      ...
      if (!box) return
      const picks = MATCH[picked] ?? MATCH.ai ?? []
      box.innerHTML = picks.flatMap(([slug, why]) => {
        const b = builderBySlug(slug)
        if (!b) return []
        return ['<a class="pick" href="/builder?b=' + b.slug + '">'
          + '<img src="' + b.avatar + '" alt="' + b.name + ' 프로필 사진">'
          + '<div><b>' + b.name + '</b><span>' + b.role + ' · ' + b.done + '건 수행</span></div>'
          + '<span class="why">' + why + '</span></a>']
      }).join('')
    }
        ...
        <div className="page-head">
          <div className="wrap">
            ...
            <div className="head-stats rv">
              ...
              <div className="hstat"><b className="num" data-count>{pad2(WORKS.length)}</b><i className="unit">건</i><span>공개 프로젝트</span></div>
              <span className="hstat-sep" aria-hidden="true">✳</span>
          ...
          <section className="panel panel--sand" id="works">
            <div className="panel__head">
              <div>
                ...
                <h2>수행 프로젝트 <span className="head-cnt num" data-cnt>( {pad2(WORKS.length)} )</span></h2>
            ...
            <div className="pxg" data-list>
              {WORKS.map(w => (
                <Link className="wcard" href={`/work/${w.slug}`} data-c={w.cat} data-cursor="VIEW →" key={w.slug}>
                  <div className="slot mask">
                    <img className="cover" src={`/assets/img/${w.cover}`} alt={w.coverAlt} loading="lazy" />
                  </div>
                  <div className="meta">
                    <div className="mrow"><span className="tag">{w.tag}</span><span className="yr num">{w.year}</span></div>
                    <h3>{w.title}</h3>
                    <p>{w.summary}</p>
                    <div className="builders">{creditOf(w)}</div>
                  </div>
                </Link>
              ))}
            </div>
          ...
          <section className="panel panel--tint" id="builders">
            <div className="panel__head">
              <div>
                ...
                <h2>검증된 빌더 <span className="head-cnt num">( {pad2(BUILDERS.length)} )</span></h2>
                ...
                return (
                  <Link className={b.box} href={`/builder?b=${b.slug}`} data-cursor="PROFILE →" data-track="builder_click" data-slug={b.slug} key={b.slug}>
                    <div className="slot mask">
                      ...
                      <div className="ct"><span className="cnt">수행 <b className="num">{b.done}</b>건</span><span className="go">Profile →</span></div>
